...
robot_load.n_ext(:,end)=zeros(3,1); 
robot_load.m_ext(:,end)=zeros(3,1);
robot_load.s_n_ext(end)=1;
robot_load.s_m_ext(end)=1;
...
for i_seg = 1:robot.n_segments
    ...
    for i_node_seg = 1:segment.n_nodes_seg-1
        ...
        [y_contact,f_temp_n,f_temp_t,x_contact,gap] = contact_handling(objects, y_seg(:,i_node_seg+1), segment.segment_radius, prev_state(:,i_node-1+i_node_seg+1));%, b_isTip);%(objects, y_seg(:,i_node_seg+1), i_node_seg, i_seg, segment.n_nodes_seg, robot.n_segments, segment.segment_radius, prev_state(:,i_node-1+i_node_seg+1));
        if any(y_contact)
            ...
            %Contact force added as external force for subsequent node
            robot_load.n_ext(:,end)=y_contact(1:3); 
            robot_load.m_ext(:,end)=y_contact(4:6);
            robot_load.s_n_ext(end)=s_vec(i_node_seg+1);
            robot_load.s_m_ext(end)=s_vec(i_node_seg+1);
        end
    end
...
%     residual = [residual; 1/robot.segments{robot.n_segments}.segment_radius*(bc.mL - mL)];
